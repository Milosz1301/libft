# libft
My implementation of some of the standard C library functions including some additional ones.

### TOC
* [What is libft?](#what-is-libft)
* [What's in it?](#whats-in-it)
* [How does it work?](#how-does-it-work)

### What is libft?
Libft is an individual project at 42 that requires us to re-create some standard C library functions for future use, and have a deeper understanding of data structures and basic algorithms. At 42 we are not allowed to use some standard libraries on our projects, so we have to keep updating our libft with functions we think will help us during our journey.

### What's in it?

As of today, the following functions are implemented:

**Standard C functions**
* [`ft_memset`](ft_memset.c)
* [`ft_bzero`](ft_bzero.c)
* [`ft_memcpy`](ft_memcpy.c)
* [`ft_memccpy`](ft_memccpy.c)
* [`ft_memmove`](ft_memmove.c)
* [`ft_memchr`](ft_memchr.c)
* [`ft_memcmp`](ft_memcmp.c)
* [`ft_strlen`](ft_strlen.c)
* [`ft_strlcpy`](ft_strlcpy.c)
* [`ft_strlcat`](ft_strlcat.c)
* [`ft_strchr`](ft_strchr.c)
* [`ft_strrchr`](ft_strrchr.c)
* [`ft_strnstr`](ft_strnstr.c)
* [`ft_strncmp`](ft_strncmp.c)
* [`ft_atoi`](ft_atoi.c)
* [`ft_isalpha`](ft_isalpha.c)
* [`ft_isdigit`](ft_isdigit.c)
* [`ft_isalnum`](ft_isalnum.c)
* [`ft_isascii`](ft_isascii.c)
* [`ft_isprint`](ft_isprint.c)
* [`ft_toupper`](ft_toupper.c)
* [`ft_tolower`](ft_tolower.c)
* [`ft_calloc`](ft_calloc.c)
* [`ft_strdup`](ft_strdup.c)

**Non-standard functions**
* [`ft_substr`](ft_substr.c)
* [`ft_strjoin`](ft_strjoin.c)
* [`ft_strtrim`](ft_strtrim.c)
* [`ft_split`](ft_split.c)
* [`ft_itoa`](ft_itoa.c)
* [`ft_strmapi`](ft_strmapi.c)
* [`ft_putchar_fd`](ft_putchar_fd.c)
* [`ft_putstr_fd`](ft_putstr_fd.c)
* [`ft_putendl_fd`](ft_putendl_fd.c)
* [`ft_putnbr_fd`](ft_putnbr_fd.c)

**Linked list functions**
* [`ft_lstnew`](ft_lstnew.c)
* [`ft_lstadd_front`](ft_lstadd_front.c)
* [`ft_lstsize`](ft_lstsize.c)
* [`ft_lstlast`](ft_lstlast.c)
* [`ft_lstadd_back`](ft_lstadd_back.c)
* [`ft_lstdelone`](ft_lstdelone.c)
* [`ft_lstclear`](ft_lstclear.c)
* [`ft_lstiter`](ft_lstiter.c)
* [`ft_lstmap`](ft_lstmap.c)

### How does it work?

The library is written in C and needs the `gcc` compiler, with `<stdlib.h>` and `<unistd.h>` standard libraries to run.

**Instruction**

1.  **Compiling the library**

To compile the library, go to its path and run:

```bash
$ make
